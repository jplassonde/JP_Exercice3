...
_I2C_bData:
 I2C_bData:                            blk   1  ; Data transfered
_I2C_bXferCnt:
 I2C_bXferCnt:                                   ; Reused variable for counting transfered
_I2C_bAddr:                                      ; Bytes
 I2C_bAddr:                            blk   1  ; Slave Address
_I2C_bit_count:
 I2C_bit_count:                        blk   1  ; Bit counter
...
_I2C_Start:
    ...
    or   [Port_0_DriveMode_1_SHADE],I2C_SDA_pin|I2C_SCL_pin
    and  [Port_0_DriveMode_0_SHADE],~(I2C_SDA_pin|I2C_SCL_pin)

IF(I2C_DriveMode2)            
    and  [Port_0_DriveMode_2_SHADE],~(I2C_SDA_pin|I2C_SCL_pin)
    mov  A,[Port_0_DriveMode_2_SHADE]
    mov  reg[I2C_I2CPRTDM2],A
ENDIF

	M8C_SetBank1
    mov  A,[Port_0_DriveMode_1_SHADE]
    mov  reg[I2C_I2CPRTDM1],A
    mov  A,[Port_0_DriveMode_0_SHADE]
    mov  reg[I2C_I2CPRTDM0],A
    M8C_SetBank0
                                                     ; Set both pins, low so that only the port
                                                     ; mode has to be changed from a 0 to a 1
    and  [Port_0_Data_SHADE],~(I2C_SDA_pin|I2C_SCL_pin)
    mov  A,[Port_0_Data_SHADE]
    mov  reg[I2C_I2CPRTDR],A
...
CheckRxAck:                                                ; Test to see if Slave ACKed
    RAM_SETPAGE_CUR >I2C_bStatus
    tst   [I2C_bStatus],I2C_SLAVE_ACKed
    jz    End_RD_Loop
                                                           ; Send ACK until last byte
    or    [I2C_bStatus],I2C_SEND_ACK                       ; Send ACK until last byte
RD_Loop:                                                   ; Loop through bytes
    push  x                                                ; Save X for later
    dec   [x+RxByteCount]                                  ; Decrement data counter
    jnz   I2C_READ
    and   [I2C_bStatus],~I2C_SEND_ACK                      ; If last Byte, don't ACK
I2C_READ:
...
ENDIF
    ...
    mov   a,[I2C_bData]                                    ; Save data in data array
...
End_RD_Loop:                                               ; Endloop
    tst   [x+RxMode],I2C_NoStop
    jnz   SkipRxStop
    call  I2C_SendStop
SkipRxStop:
    RAM_SETPAGE_CUR >I2C_bStatus
    mov   a,I2C_XFER_ERROR                                 ; Clear return value until we test if slave ACKed
    tst   [I2C_bStatus],I2C_SLAVE_ACKed                    ; Test to see if Slave ACKed
    jz    End_ReadBytes
    mov   a,I2C_XFER_COMPLETE                              ; Return ACKed True
End_ReadBytes:
...
DoTxAck:
    RAM_SETPAGE_CUR >I2C_bStatus
    mov   [I2C_bXferCnt],0x00                         ; Zero out byte count first
    tst   [I2C_bStatus],I2C_SLAVE_ACKed               ; Test to see if Slave ACKed
    jz    End_WR_Loop
...
_I2C_bWriteCBytes:
    ...
    tst   [x+TxCMode],I2C_RepStart                    ; Check if a Start or RepeatStart
...
TxCMode_x:       equ  -3
 ...
 I2C_bWriteCBytes:					        
    ...
    tst   [x+TxCMode_x],I2C_RepStart                  ; Check if a Start or RepeatStart
...
I2C_DoRepeatStart:
    ...
    push  a
    M8C_SetBank1
    or    [Port_0_DriveMode_0_SHADE],I2C_SCL_pin           ; Assert SCL low
    and   [Port_0_DriveMode_0_SHADE],~I2C_SDA_pin          ; Release SDA high
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A
    ...
    call  I2C_delay
                                                           ; Release SCL high
    and   [Port_0_DriveMode_0_SHADE],~I2C_SCL_pin
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A
...
clockstretch1:                                             ; Wait for SCL high in case of wait
    ...
    tst   reg[I2C_I2CPRTDR],I2C_SCL_pin
    jz    clockstretch1

    mov   A,I2C_delay3qtr                                  ; Delay 1/2 period
    call  I2C_delay
    pop   a
...
I2C_DoStart:
    ...
    or    [Port_0_DriveMode_0_SHADE],I2C_SDA_pin                ; Assert SDA low
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A

    mov   A,I2C_delayhalf                                       ; Delay 1/2 period
    call  I2C_delay
    pop   A                                                     ; Restore slave address to A
    M8C_SetBank0
    RAM_SETPAGE_CUR >I2C_bData
    mov   [I2C_bData],A

    RAM_SETPAGE_CUR >Port_0_DriveMode_0_SHADE
    M8C_SetBank1
    or    [Port_0_DriveMode_0_SHADE],I2C_SCL_pin                ; Set SCL low
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A
    M8C_SetBank0

    mov   A,I2C_delayqtr                                        ; Delay 1/4 period
    call  I2C_delay
    call   I2C_write                                            ; Write the byte.
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
.ENDSECTION
...
;          CUR_PP
;
;    Send data byte to slave. Must be followed by I2C_get_ack.
;
I2C_fWrite:
_I2C_fWrite:
    RAM_PROLOGUE RAM_USE_CLASS_4
    RAM_SETPAGE_CUR >I2C_bData
    mov   [I2C_bData],A                                    ; Put data in bData
    call  I2C_write
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
_I2C_write:
    RAM_PROLOGUE RAM_USE_CLASS_4
    RAM_SETPAGE_CUR >I2C_bit_count
    
    mov   [I2C_bit_count],8                                 ; Set up count to xmit 8 bits
writenextbit:
    dec   [I2C_bit_count]
    ...
    RAM_SETPAGE_CUR >Port_0_DriveMode_0_SHADE
                                                           ; Assert SCL low
    M8C_SetBank1
    or    [Port_0_DriveMode_0_SHADE],I2C_SCL_pin
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A
    M8C_SetBank0
    ...
    or    [Port_0_DriveMode_0_SHADE],I2C_SDA_pin
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A

    jmp   writestartclock
write_bit:                                                 ; Release SDA high for data 1
    and   [Port_0_DriveMode_0_SHADE],~I2C_SDA_pin      
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A
...
;
;  DESCRIPTION:
;    Get slave acknowledge response. Used only by I2C_write.
;
;-----------------------------------------------------------------------------
;
;  ARGUMENTS:
...
clockstretch3:                                             ; Wait for SCL to go high in case wait states
    ...
    tst   reg[I2C_I2CPRTDR],I2C_SDA_pin
...
getack2:
    ...
    mov   A,[I2C_bStatus]                                  ; Get result and put it in A
    and   A,I2C_SLAVE_ACKed                                ; Mask off just the ACK flag.
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
.ENDSECTION
...
_I2C_bRead:
    ...
    jz    I2C_read                                         ; Don't ACK
    or    [I2C_bStatus],I2C_SEND_ACK                       ; Set ACK flag
    call  I2C_read
    RAM_EPILOGUE RAM_USE_CLASS_4
    ret
...
_I2C_read:
    RAM_PROLOGUE RAM_USE_CLASS_4
    RAM_SETPAGE_CUR >I2C_bStatus    
    mov   [I2C_bit_count],8                                ; Count and prepare bits to read
readnextbit:
    dec   [I2C_bit_count]
    rlc   [I2C_bData]
...
clockstretch4:                                             ; Wait for SCL to go high in case slave
    ...
    and   [I2C_bData],~01h                                 ; Record bit received
    tst   reg[I2C_I2CPRTDR],I2C_SDA_pin
    jz    readnobit
    or    [I2C_bData],01h
readnobit:
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: I2C_put_ack
;
;  DESCRIPTION:
;    Send acknowledge to slave. Used only by I2C_read
;
;-----------------------------------------------------------------------------
;
;  ARGUMENTS:
;    I2C_bStatus - Set for no ack to be followed by stop. Clear for ack
...
_I2C_SendStop:
    ...
    or    [Port_0_DriveMode_0_SHADE],I2C_SCL_pin|I2C_SDA_pin
    mov   A,[Port_0_DriveMode_0_SHADE]
    mov   reg[I2C_I2CPRTDM0],A

    mov   A,I2C_delay3qtr                                  ; Delay 3/4 period
    call  I2C_delay
                                                           ; Release SCL high
    and   [Port_0_DriveMode_0_SHADE],~I2C_SCL_pin
    mov   A,[Port_0_DriveMode_0_SHADE]
...
;
;  DESCRIPTION:
;    Create delays for I2C routines.
;
;-----------------------------------------------------------------------------
;
;  ARGUMENTS: none
...
;    functions.
;          
I2C_delay:                                       ; 11 cycles for call. 32 total.
I2Cdelay1:                                       ; Delay 1/4 period at 100 kHz
     dec  A                                      ; 4 cycles
     jnz  I2Cdelay1                              ; 5 cycles for jump, 4 no jump.     
     ret                                         ; 8 cycles
